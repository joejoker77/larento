    ...
    private function checkImages(PostRequest $request, Post $post): void
    {
        if ($images = $request->file('photo')) {
            foreach ($images as $i => $image) {
                if ($post->photos()->where("name", '=', str_replace('.'.$image->getClientOriginalExtension(), '', $image->getClientOriginalName()) . '.webp')->first()) {
                    continue;
                }
                ...
                save_image_to_disk($image, $photo, Post::getImageParams()['sizes'], $post->category->slug == 'portfolio' ? true : null);
            }
        }
    }
}
