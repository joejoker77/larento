        ...
        <div class="row">
            <div class="col-8">
                <table class="table-bordered table table-striped">
                    ...
                    <tr>
                        <th>Как опция</th>
                        <td>@if($attribute->mode == $attribute::MODE_AS_OPTION)
                                Да
                            @else
                                Нет
                            @endif</td>
                            ...
                            @foreach($attribute->variants as $i => $variant)
                                @if(strpbrk($variant, '|'))
                                    @php
                                        ...
                                        $value      = $arrayValue[1];
                                    @endphp
                                    {{ $variant }} <span style="display: inline-block;background-color:{{trim('#'.$value)}};width: 16px;height: 16px;border: 1px solid #000000"></span>@if((count($attribute->variants) - 1) !== $i),@endif
                                @else
                ...
                @if(!$categories->isEmpty())
                    <form action="{{ route('admin.shop.attributes.assign-categories', $attribute) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="categories[]" class="js-choices" multiple data-action="{{ route('admin.shop.attributes.un-assign-category', $attribute) }}" data-placeholder="-=Выбрать категории=-">
                                <option></option>
