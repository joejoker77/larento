...
@section('content')
    <div class="pt-4 d-flex">
        <div class="ms-auto btn-group" role="group" aria-label="control buttons">
            ...
            @if($product->isActive())
                <form class="btn btn-primary" method="POST" action="{{ route('admin.shop.product.set-un-active', $product) }}" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Снять с публикации">
                    @csrf
            ...
            @else
                <form class="btn btn-primary" method="POST" action="{{ route('admin.shop.product.set-active', $product) }}" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Опубликовать">
                    @csrf
                    <button type="submit" class="btn p-0 text-white d-flex" style="line-height: 0">
                    ...
                    <tr>
                        ...
                        <td>
                            @if(strpbrk($value->value, '|'))
                                @if(strpbrk($value->value, ','))
                                    @php $arrayValues = explode(',', $value->value) @endphp
                                    @foreach($arrayValues as $i => $val)
                                        ...
                                        @endphp
                                        {{ $variant }} <span style="display: inline-block;background-color:{{trim('#'.$valueStr)}};width: 16px;height: 16px;border: 1px solid #000000"></span>@if((count($arrayValues) - 1) !== $i),@endif
                                    @endforeach
                                ...
                                @endif
                            @elseif($value->attribute::TYPE_BOOLEAN == $value->attribute->type)
                                Да
                            @else
                                {{ $value->value }}
                            @endif
