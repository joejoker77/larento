...
@section('content')
        ...
        <div class="ms-auto">
            <form class="p-0 m-0" method="POST" id="formActions"
                ...
                <div class="btn-group" role="group" aria-label="control buttons">
                    ...
                    <button type="submit" name="action" value="main-page" class="btn btn-lg btn-success" style="line-height: 0"
                            data-bs-toggle="tooltip" data-bs-placement="bottom"
                            data-bs-title="Отобразить на главной">
                ...
                <div class="btn-group" role="group" aria-label="control buttons">
                    ...
                    <button type="submit" name="action" value="revoke-main-page" class="btn btn-lg btn-success" style="line-height: 0"
                            data-bs-toggle="tooltip" data-bs-placement="bottom"
                            data-bs-title="Убрать с главной главной">
    ...
    <table class="table table-bordered table-striped" id="itemsTable">
        ...
        <tr>
            <form action="{{request()->fullUrl()}}" name="search-products" method="GET" id="searchItems"></form>
            <td>&nbsp;</td>
            ...
            <td style="max-width: 175px">
                <input type="text" form="searchItems" name="sku" class="form-control" aria-label="Искать по артикулу" value="{{ request('sku') }}">
            </td>
            <td style="width: 250px">
                <select name="category" id="selectCategory" class="js-choices" form="searchItems" data-placeholder="-= Выбрать категорию =-">
                    <option></option>
                    @if($categories)
            ...
            <td style="width: 200px">
                <select name="status" form="searchItems" class="js-choices" id="selectStatus" data-placeholder="-= Выбрать статус =-">
                    <option></option>
            ...
            <td>
                <label class="plus-minus-checkbox">
                    <input type="checkbox" name="hit" form="searchItems" value="true" @checked(request('hit'))>
                    <span class="value"></span>
                </label>
            </td>
            <td>
                <label class="plus-minus-checkbox">
                    <input type="checkbox" name="new" form="searchItems" value="true" @checked(request('new'))>
                    <span class="value"></span>
                </label>
            </td>
            <td>
                <label class="plus-minus-checkbox">
                    <input type="checkbox" name="display_home" form="searchItems" value="true" @checked(request('display_home'))>
                    <span class="value"></span>
                </label>
            </td>
            <td>&nbsp;</td>
        ...
        @foreach($products as $product)
            <tr>
                ...
                <td style="text-align: center;">
                    <span class="badge @if($product->display_home) text-bg-success @else text-bg-secondary @endif">@if($product->display_home) Да @else Нет @endif</span>
                ...
                <td style="white-space: nowrap">
                    ...
                    >
                    ...
                    </a>|<form method="POST" action="{{ route('admin.shop.products.set-status') }}" class="list-inline-item mx-1" data-bs-placement="bottom"
                               data-bs-toggle="tooltip" data-bs-title="@if($product->status == $product::STATUS_ACTIVE)Снять с публикации@elseОпубликовать@endif">
                        @csrf
                    ...
                    </form>|<form method="POST" action="{{ route('admin.shop.products.set-status') }}" class="list-inline-item mx-1" data-bs-placement="bottom"
                                  data-bs-toggle="tooltip" data-bs-title="@if($product->hit)Удалить из хитов продаж@elseОтметить как хит продаж@endif">
                        @csrf
                        <input type="hidden" name="action"
                        ...
                        <button class="btn p-0 align-baseline text-primary" type="submit">
                            ...
                            @endif
                                  ...
                                  data-bs-toggle="tooltip" data-bs-title="@if($product->new)Удалить из новинок@elseОтметить как новинку@endif">
                        @csrf
                        <input type="hidden" name="action"
                        ...
                        <button class="btn p-0 align-baseline text-primary" type="submit">
                            ...
                            @endif
                                  ...
                                  data-bs-toggle="tooltip" data-bs-title="@if($product->display_home)Убрать с главной@elseОтобразить на главной@endif">
                        @csrf
                        <input type="hidden" name="action"
                               value=@if($product->display_home)"revoke-main-page"@elseif(!$product->display_home)"main-page"@endif">
                        <input type="hidden" name="selected[]" value="{{ $product->id }}">
                        <button class="btn p-0 align-baseline text-primary" type="submit">
                            @if($product->display_home)
                                <span data-feather="toggle-right"></span>
                            @else
                                <span data-feather="toggle-left"></span>
                            @endif
