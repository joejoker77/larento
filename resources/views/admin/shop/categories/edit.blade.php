...
@section('content')
    <div class="my-4">
        ...
        <form id="editForm" method="POST" action="{{ route('admin.shop.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            @if(!$categories->isEmpty() || $category->allAttributes()->isNotEmpty())
                <div class="p-3 mb-3 bg-light border">
                    <div class="row">
                        @if(!$categories->isEmpty())
                            <div class=@if($category->allAttributes()->isNotEmpty())"col-md-6"@else"col-md-12"@endif>
                                <h4 class="my-3 pb-3 border-bottom">Родительская категория</h4>
                                @error('parent_id')<div class="is-invalid"></div>@enderror
                                <select name="parent_id" class="js-choices" data-placeholder="-=Привязать категорию=-" aria-label="Родительская категория">
                                    <option></option>
                        ...
                        @if(!$attributes->isEmpty())
                            <div class=@if(!$categories->isEmpty())"col-md-6"@else"col-md-12"@endif>
                                ...
                                <select name="attributes[]" class="js-choices" multiple data-placeholder="-=Выбрать аттрибуты=-" aria-label="Привязка атрибутов">
                                    <option></option>
