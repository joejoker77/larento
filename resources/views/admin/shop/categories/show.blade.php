...
@section('content')
    <div class="pt-4 d-flex">
        <div class="ms-auto btn-group" role="group" aria-label="control buttons">
            ...
            <a href="{{ route('admin.shop.categories.edit', $category) }}" class="btn btn-warning d-flex" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Редактировать">
                <span data-feather="edit"></span>
            </a>

            <form class="btn btn-info" method="POST"
                ...
                <table class="table table-striped table-bordered">
                    ...
                    @if($attributes->isNotEmpty())
                        <tr>
                            <th>Аттрибуты:</th>
                            <td>
                                @foreach($attributes->sortBy('id') as $attribute)
                                    {{ $attribute->name }}@if($attributes->count() !== $attribute->id),@endif
                                @endforeach
                            </td>
                        </tr>
                    @endif
