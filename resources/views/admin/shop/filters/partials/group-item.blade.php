    ...
    <div id="collapse-@if($group){{ $group->id }}@endif" class="accordion-collapse collapse @if($key === 0)show @endif" aria-labelledby="heading-@if($group){{ $group->id }}@endif" data-bs-parent="#accordionGroups">
        <div class="accordion-body">
            ...
            <div class="form-floating mb-3">
                ...
                <select name="group_categories[@if($group){{ $group->id }}@endif][]" class="js-choices" multiple data-placeholder="-=Выбрать категорию=-">
                    <option></option>
            ...
            <div class="form-floating mb-3">
                <h5>Теги в группе</h5>
                <select name="tags[@if($group){{ $group->id }}@endif][]" class="js-choices" multiple data-placeholder="-=Выбрать теги=-">
                    <option></option>
                    @foreach($tags as $tag)
                        <option value="{{ $tag->id }}"
                        @if($group && $group->tags)
            ...
            <div class="form-floating mb-3">
                ...
                <select name="attributes[@if($group){{ $group->id }}@endif][]" class="js-choices" multiple data-placeholder="-=Выбрать атрибуты=-">
                    <option></option>
